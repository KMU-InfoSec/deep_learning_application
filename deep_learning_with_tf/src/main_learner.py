...
def get_data_path(model_arg):
    input_data_type = model_arg['input_data_type']
    # realconfig.ini 의 [BASIC_INFO]->'INPUT_DATA_TYPE' 설명 참고
    ...
    if input_data_type == '1':
        # data
        mal_data = np.array(walk_dir(model_arg['mal_dir'], model_arg['fh_type']))
        ben_data = np.array(walk_dir(model_arg['ben_dir'], model_arg['fh_type']) if model_arg['class_type'] == 'binary' else list())
        ...
        # data
        train_mal_data = np.array(walk_dir(model_arg['mal_dir'], model_arg['fh_type']))
        train_ben_data = np.array(walk_dir(model_arg['ben_dir'], model_arg['fh_type']))
        test_mal_data, test_ben_data = read_test_data(model_arg['test_dir'], model_arg['label_path'], model_arg['fh_type'])
        ...
        mal_data, ben_data, indices_obj = split_train_test_data_to_date(class_type=model_arg['class_type'],
                                                        ...
                                                        ben_ratio='4:1', ext=model_arg['fh_type'],
...
def main():
    model_arg = {
        ...
        'input_data_type': config.get('BASIC_INFO', 'INPUT_DATA_TYPE'),
